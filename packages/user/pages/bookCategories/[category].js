...
const Category = ({ user, }) => {
  ...
  const { data, error } = useSWR(`${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/books?category=${linkRouter}`, unauthorFetch)
  ...
  if (!data) return <div style={{ width: '100%', height: '100%', margin: '0 auto' }}><h2>Loading...</h2></div>
  if (error) return `${error}`
  return (

    data && dataDetail.length == 0 ? <div>
      <h2 style={{
        position: 'absolute', left: '50%', top: '50%',color:'#00bdd7',
      ...
      <div>
        ...
        >
          {dataDetail
            ...
            .map((element, ind) => {
              if (ind <= 3) {
                return (<Grid item xs={12} sm={6} md={4} lg={3} key={ind}>
